...
- อ่าน Discord webhook จาก `DISCORD_WEBHOOK`, `Botdiscord.rtf`, `Botdiscord.txt`, `Botdiscord` หรือ `URLBot.txt`
...
```text
Login UI -> Binance API Key -> Discord Alert Bot URL -> Dashboard index
```

เปิดดูสถานะ bot:

```text
http://localhost:8787/bot
```
...
- `ALERT_PORT=8787`
- `DISCORD_WEBHOOK=` ใช้แทนไฟล์ webhook เวลารันบน cloud
- `AI_PROVIDER=none`
- `AI_MODE=alert_only`
...
- `SESSION_SECRET=` ให้ Render generate หรือใส่ random string ยาว ๆ

## Login user database

ระบบ login จะอ่านจากไฟล์ `auth-users.json` เท่านั้น:
...
ถ้าต้องการเก็บแบบ hash ให้สร้าง hash:

```bash
npm run hash-password -- "รหัสผ่านของคุณ"
```

แล้วเอาค่าที่ได้ไปใส่แบบนี้:
...
หมายเหตุ: `auth-users.json` ถูกใส่ใน `.gitignore` เพื่อกันรหัสหลุดขึ้น git.
ถ้ารันบน Render แนะนำสร้างเป็น Secret File ชื่อ `auth-users.json`
หรือถ้า repo เป็น private และยอมรับความเสี่ยงเอง ค่อย force add ไฟล์นี้ขึ้น repo.

## Render Web Service

ใช้ไฟล์ `render.yaml` ได้เลย หรือสร้าง Web Service เองด้วยค่านี้:
...
```text
https://<render-app>.onrender.com/login
```

ลำดับการเข้าใช้งาน:

1. Login ด้วย user/password จาก `auth-users.json`
2. กรอก Binance API Key และ Secret Key ในหน้า setup
3. กรอก Discord Webhook URL ในช่อง `Discord Webhook URL`
4. กด `Connect Dashboard`
5. หน้า dashboard จะเปิดที่ `/` และหน้า status bot อยู่ที่ `/bot`

Discord webhook ที่กรอกใน UI จะถูกบันทึกฝั่ง server เป็น `.discord-webhook`
เพื่อให้ alert bot ส่ง Discord ได้แม้ไม่ได้เปิดหน้าเว็บค้างไว้.
...
หมายเหตุ: อย่า public ไฟล์ `URLBot.txt` / `Botdiscord*` และอย่าเอา webhook ไปใส่ในหน้าเว็บฝั่ง client
