...
const WEBHOOK_FILES = ['Botdiscord.rtf', 'Botdiscord.txt', 'Botdiscord', 'URLBot.txt'];
const AUTH_USERS_PATH = path.join(ROOT, process.env.AUTH_USERS_FILE || 'auth-users.json');
const RUNTIME_WEBHOOK_PATH = path.join(ROOT, '.discord-webhook');
...
async function readAuthDb() {
  const raw = await fs.readFile(AUTH_USERS_PATH, 'utf8');
  const db = JSON.parse(raw);
  const users = Array.isArray(db.users) ? db.users : [];
  return users
    .filter(u => u && u.enabled !== false && typeof u.username === 'string')
    .map(u => ({
...
async function authDbStatus() {
  try {
    const users = await readAuthDb();
    return { configured: users.length > 0, users, error: '' };
  } catch (err) {
    return { configured: false, users: [], error: err.code === 'ENOENT' ? `Missing ${path.basename(AUTH_USERS_PATH)}` : err.message };
  }
}
...
function verifyPasswordHash(password, encoded) {
  ...
  const [, digest, iterText, saltB64, hashB64] = parts;
...
async function verifyLogin(username, password) {
  ...
  if (!user) return null;
  if (user.passwordHash && verifyPasswordHash(password, user.passwordHash)) return user;
  if (user.password && safeEqual(user.password, password)) return user;
  return null;
}
...
async function readWebhook() {
  ...
  const envMatch = extractDiscordWebhook(envUrl);
  if (envMatch) {
    bot.webhookLoaded = true;
    return envMatch;
  }
  try {
    const runtimeUrl = extractDiscordWebhook(await fs.readFile(RUNTIME_WEBHOOK_PATH, 'utf8'));
    if (runtimeUrl) {
      bot.webhookLoaded = true;
      return runtimeUrl;
    }
  } catch (err) {
    if (err.code !== 'ENOENT') console.warn(`[webhook] ${err.message}`);
  }
  const errors = [];
  for (const file of WEBHOOK_FILES) {
    try {
      const raw = await fs.readFile(path.join(ROOT, file), 'utf8');
      const url = extractDiscordWebhook(raw);
      if (url) {
        bot.webhookLoaded = true;
        return url;
      }
      errors.push(`${file}: no Discord webhook URL`);
    } catch (err) {
      if (err.code !== 'ENOENT') errors.push(`${file}: ${err.message}`);
    }
  }
  throw new Error(`Set DISCORD_WEBHOOK or put a Discord webhook URL in ${WEBHOOK_FILES.join(' / ')}${errors.length ? ` (${errors.join('; ')})` : ''}`);
}

async function saveWebhook(url) {
  const clean = extractDiscordWebhook(url);
  if (!clean) throw new Error('Invalid Discord webhook URL');
  await fs.writeFile(RUNTIME_WEBHOOK_PATH, `${clean}\n`, { mode: 0o600 });
  bot.webhookLoaded = true;
  bot.lastError = bot.lastError && bot.lastError.includes('Discord webhook') ? '' : bot.lastError;
  return clean;
}

function extractDiscordWebhook(text) {
  return String(text || '').match(/https:\/\/discord(?:app)?\.com\/api\/webhooks\/[^\s\\}"]+/)?.[0] || '';
}
...
function requireAuth(req, res) {
  if (req.url === '/health' || req.url.startsWith('/login')) return true;
  if (getSession(req)) return true;
  if (req.url.startsWith('/api/')) return json(res, 401, { ok: false, error: 'login required' });
  redirect(res, '/login');
  return false;
}
...
function loginPage(error = '', authStatus = { configured: false, error: '' }) {
  return htmlShell('Login', `
    <div style="min-height:calc(100vh - 40px);display:grid;place-items:center">
      <form method="POST" action="/login" class="box" style="width:min(420px,100%);padding:22px">
        <h1>Binance Futures Login</h1>
        <div class="muted" style="line-height:1.6;margin-bottom:16px">เข้าสู่ระบบก่อนใช้งาน dashboard บน Render</div>
        ${authStatus.configured ? '<div class="box ok" style="margin-bottom:12px">Loaded users from auth-users.json</div>' : `<div class="box bad" style="margin-bottom:12px">${escapeHtml(authStatus.error || 'auth-users.json has no enabled users')}</div>`}
        ${error ? `<div class="box bad" style="margin-bottom:12px">${escapeHtml(error)}</div>` : ''}
        <label class="k">User</label>
        <input name="user" autocomplete="username" autofocus style="width:100%;margin:6px 0 12px;padding:12px;border-radius:6px;border:1px solid var(--line);background:#111824;color:var(--text)">
        <label class="k">Password</label>
        <input name="password" type="password" autocomplete="current-password" style="width:100%;margin:6px 0 16px;padding:12px;border-radius:6px;border:1px solid var(--line);background:#111824;color:var(--text)">
        <button style="width:100%">Login</button>
        <div class="muted" style="font-size:12px;line-height:1.6;margin-top:14px">Login source: <b>${escapeHtml(path.basename(AUTH_USERS_PATH))}</b> only. Env ที่ต้องตั้งบน Render: <b>SESSION_SECRET</b></div>
      </form>
    </div>
  `, false);
}
...
async function serveStaticFile(res, file, contentType) {
  try {
    ...
    json(res, err.code === 'ENOENT' ? 404 : 500, { ok: false, error: err.message });
  }
}
...
async function historyPage() {
  ...
  return htmlShell('Paper Trade History', `
    <div class="top">
      ...
      <div class="actions">
        <a class="btn secondary" href="/">Dashboard</a>
        <a class="btn secondary" href="/bot">Bot Status</a>
